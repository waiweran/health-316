...
def gdpCorrelation(condition_name, data_type, year, min_age=-1, max_age=-1, gender='all', race_ethnicity='all'):
    '''Gets a dataset in map plotting format given a condition name and a data type'''
    conn = psycopg2.connect("dbname=health")
    c = conn.cursor()
    c.execute('''
        SELECT AVG(POWER(datapoint.value - location.gdp))
        FROM datapoint, condition_name, location
        WHERE datapoint.condition_id = condition_name.condition_id
        AND datapoint.location_id = location.uid
        AND condition_name.name = %s
...
def incomeCorrelation(condition_name, data_type, year, min_age=-1, max_age=-1, gender='all', race_ethnicity='all'):
    '''Gets a dataset in map plotting format given a condition name and a data type'''
    conn = psycopg2.connect("dbname=health")
    c = conn.cursor()
    c.execute('''
        SELECT AVG(POWER(datapoint.value - location.avg_income))
        FROM datapoint, condition_name, location
        WHERE datapoint.condition_id = condition_name.condition_id
        AND datapoint.location_id = location.uid
        AND condition_name.name = %s
